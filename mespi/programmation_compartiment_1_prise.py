...
class Programmation_compartiment_View(QWidget):
    def __init__(self, fenetre_selection, controller):
        super().__init__()

        self.fenetre_selection = fenetre_selection   
        self.myController = controller
        programmation_compartiment_2_prises_Model = Programmation_compartiment_2_prises_Model()
        programmation_compartiment_2_prises_Controller = Programmation_compartiment_2_prises_Controller(programmation_compartiment_2_prises_Model)
        self.programmation_medicament_2_prises = Programmation_compartiment_View_2(self, fenetre_selection, programmation_compartiment_2_prises_Controller)
    ...
    def btn_ajout_prise(self):
        self.hide()
        self.programmation_medicament_2_prises.show()
        ...
        self.checkbox_dimanche.isChecked(), self.prise_1.text())
        self.programmation_medicament_2_prises.compartiment_selectionne(self.label_numero_compartiment.text())

    def btn_retour(self):
        self.close()
        self.fenetre_selection.remplir_nom()
        self.fenetre_selection.show()
        if self.nom_medicament.text() != "":
                self.myController.enregistrer(self.label_numero_compartiment.text(), self.nom_medicament.text(), 
                self.checkbox_lundi.isChecked(), self.checkbox_mardi.isChecked(), self.checkbox_mercredi.isChecked(),
                self.checkbox_jeudi.isChecked(), self.checkbox_vendredi.isChecked(), self.checkbox_samedi.isChecked(),
                self.checkbox_dimanche.isChecked(), self.prise_1.text())
...
#        print("remplir 1 prise")
#        print(data_compartiment)
        try : 
                self.nom_medicament.setText(data_compartiment[1])
                self.prise_1.setText(str(data_compartiment[9]))
                self.checkbox_lundi.setChecked(data_compartiment[2])
                self.checkbox_mardi.setChecked(data_compartiment[3])
                self.checkbox_mercredi.setChecked(data_compartiment[4])
                self.checkbox_jeudi.setChecked(data_compartiment[5])
                self.checkbox_vendredi.setChecked(data_compartiment[6])
                self.checkbox_samedi.setChecked(data_compartiment[7])
                self.checkbox_dimanche.setChecked(data_compartiment[8])
        except :
...
class Programmation_compartiment_Controller:
...
class Programmation_compartiment_Model:
        ...
        def recupere_data_compartiment_m(self, id_compartiment):
                conn = sqlite3.connect('compartiments.db')
                cursor = conn.cursor()
                cursor.execute("""SELECT id_compartiment, medicament, lundi, mardi, mercredi, jeudi, vendredi, samedi, dimanche, prise_1 FROM compartiments WHERE id_compartiment=?""", (id_compartiment,))
        ...
        def enregistrer_m(self, id_compartiment, nom_medicament, lundi, mardi, mercredi, jeudi,
                ...
                "dimanche" :dimanche, "prise_1" :prise_1}
                cursor.execute("""
                INSERT OR REPLACE INTO compartiments(id_compartiment, medicament, lundi, mardi, mercredi, jeudi, vendredi, samedi,
                dimanche, prise_1) VALUES(:id_compartiment, :medicament, :lundi, :mardi, :mercredi, :jeudi, :vendredi,
                :samedi, :dimanche, :prise_1)""", data)
                conn.commit()
                conn.close()
#                print("enregistré")
#                print(data)
